...
AGENT_CODES = {"AT", "SS", "私域", "WW", "MM", "M", "ALEN", "舅", "歐"}

# 備注黑名單：出現在獨立行的這些字串會被剔除，不併入備注。
# 「信威」是固定寫在產出檔名的中段標籤，不是代理、也不是備注內容，
# 若使用者貼的文字裡帶了「信威」就直接忽略，避免污染備注欄。
SKIP_TOKENS = {"信威"}
_SKIP_RE = re.compile(
...
def _norm_smoking(value):
    s = (value or "").strip()
    if any(k in s for k in ("不", "無", "无", "否", "没", "沒", "禁", "no", "NO", "N", "non")):
        return "不吸煙"
    return "吸煙"
